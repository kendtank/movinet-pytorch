...
def export_to_onnx_with_fixes(model, onnx_path='movinet_optimized.onnx'):
    ...
    try:
        ...
        torch.onnx.export(
            ...
            # # 设置dynamic_axes后，ONNX模型可以接受：
            # # torch.randn(1, 3, 16, 224, 224)  # batch=1, time=16
            # # torch.randn(4, 3, 16, 224, 224)  # batch=4, time=16
            # # torch.randn(1, 3, 32, 224, 224)  # batch=1, time=32
            # # torch.randn(2, 3, 8, 224, 224)   # batch=2, time=8
            # 1. 灵活性
            # 训练时可能使用batch_size=1，但推理时可能需要batch_size=4  测试需不需要
            # dynamic_axes={
            #     'input': {0: 'batch_size', 2: 'time'},
            #     'output': {0: 'batch_size'}
            # },
            # 添加这些参数来解决einops和adaptive pooling问题
        ...
        try:
            ...
            print(f"简化版ONNX模型导出完成: {onnx_path}")
            return onnx_path
...
def main():
    ...
    try:
        # 加载并优化模型
        optimized_model = load_and_optimize_model('/home/kend/Guanxin/work/workspace/movinet-pytorch/export/movinet_best.pth')
